...
class TLDetector(object):
    ...
    def get_light_state(self, world_light, distance):
        ...
        if self.create_ground_truth:
            ...
            gt_image_path = os.path.join(os.path.join(self.ground_truth_dir, '{0}'.format(state)),
    ...
    def process_traffic_lights(self):
        ...
        if world_light is not None:
            ...
            if self.lane == None:
                rospy.logwarn("Haven't received waypoints yet!!!")
            else:
                for index, waypoint in enumerate(self.lane.waypoints[self.prev_first_wpt_index:] + self.lane.waypoints[
                                                                                                   :self.prev_first_wpt_index],
                                                 start=self.prev_first_wpt_index):
                    current_wpt_distance = math.sqrt(
                        (waypoint.pose.pose.position.x - stop_line_positions[light_wp][0]) ** 2 + (
                            waypoint.pose.pose.position.y - stop_line_positions[light_wp][1]) ** 2)
