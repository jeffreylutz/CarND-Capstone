...
class WaypointLoader(object):
    ...
    def __init__(self):
        ...
        self.max_speed = self.kmph2mps(rospy.get_param('~velocity'))
    ...
    def get_velocity(self, velocity):
        return self.kmph2mps(velocity)

    def kmph2mps(selfself, velocity_kmph):
        return (velocity_kmph * 1000.0) / (60.0 * 60.0)

    def mps2kmph(selfself, velocity_kmph):
        return (velocity_kmph * 60.0 * 60.0) / (1000.0)
    ...
    def decelerate(self, waypoints):
        ...
        for wp in waypoints[:-1][::-1]:
            ...
            vel = self.kmph2mps(math.sqrt(2 * MAX_DECEL * dist))
            if vel < 1.:
                vel = 0.
